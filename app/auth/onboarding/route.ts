...
export async function POST(req: Request) {
  ...
  if (!first_name) {
    return NextResponse.json({ ok: false, error: "first_name is required" }, { status: 400 });
  }

  if (!last_name) {
    return NextResponse.json({ ok: false, error: "last_name is required" }, { status: 400 });
  }
    ...
    await pool.query(
      ...
      set
        ...
        onboarding_status = case
            when onboarding_status is null or onboarding_status = '' or onboarding_status = 'started' then 'profile_done'
            when onboarding_status in ('profile_done','tenant_done','complete') then onboarding_status
