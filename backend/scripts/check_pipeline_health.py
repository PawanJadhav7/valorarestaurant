...
async def run_check(tenant_id: str, location_id: int):
    ...
    try:
        ...
        for kpi_table in ["restaurant.daily_kpi", "restaurant.kpi_daily", "analytics.daily_kpi"]:
            try:
                ...
                if row["periods"] == 0:
                    ...
                    dim(f"Latest KPI date: {row['latest_date']}")
                    if row["latest_date"] and row["latest_date"] < today:
                        warn(f"KPIs are stale — latest is {row['latest_date']}, not today ({today})")
                break
            except Exception:
                continue
        else:
    ...
    try:
        for ins_table in ["restaurant.ai_insight", "restaurant.insight", "analytics.insight"]:
            try:
                row = await conn.fetchrow(f"""
                    SELECT
                        COUNT(*)            AS total,
                        MAX(created_at)     AS latest,
                        COUNT(*) FILTER (WHERE created_at::date = $3) AS today_count
                    FROM {ins_table}
                    WHERE tenant_id = $1
                """, tenant_id, today)
    ...
    try:
        ...
        gold_latest = await conn.fetchval("""
            SELECT MAX(order_ts) FROM restaurant.fact_order
            WHERE tenant_id = $1 AND location_id = $2
        """, tenant_id, location_id)
