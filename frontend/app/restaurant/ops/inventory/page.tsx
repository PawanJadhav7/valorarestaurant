// app/restaurant/ops/inventory/page.tsx
"use client";
...
export default function InventoryOpsPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/restaurant/inventory?${sp.toString()}`, { cache: "no-store" });
      ...
      try {
        ...
        throw new Error(`Inventory API returned non-JSON (${res.status}). BodyPreview=${text.slice(0, 140)}`);
      }
  ...
  return (
    <div className="space-y-4">
      {/* Header (same premium pattern as Sales/Labor/Overview) */}
      <SectionCard title="Inventory" subtitle="Inventory health KPIs (DIOH, waste, stockouts, variance).">
        <div className="relative pt-2">
          ...
          <div className="space-y-2 pr-[760px]">
            <div className="text-sm text-muted-foreground">Control waste + avoid stockouts. Keep DIOH tight.</div>
      ...
      {loading ? (
        ...
        <SectionCard title="No inventory data yet" subtitle="Connect an inventory source to populate these KPIs.">
          <div className="text-sm text-muted-foreground">
            {data?.notes ??
              "Upload a CSV first (daily inventory snapshot + purchases + waste), then we’ll add item-level depletion + stockout signals."}
          </div>
        </SectionCard>
      )}

      {/* MVP next steps */}
      <SectionCard title="Next" subtitle="What we’ll add once inventory series exists.">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <div className="rounded-2xl border border-border bg-background/30 p-4">
            <div className="text-sm font-semibold text-foreground">DIOH Trend</div>
            <div className="mt-1 text-sm text-muted-foreground">Days inventory on hand over time with targets.</div>
          </div>

          <div className="rounded-2xl border border-border bg-background/30 p-4">
            <div className="text-sm font-semibold text-foreground">Waste Trend</div>
            <div className="mt-1 text-sm text-muted-foreground">Waste $ and waste % (by category) with alerts.</div>
          </div>

          <div className="rounded-2xl border border-border bg-background/30 p-4">
            <div className="text-sm font-semibold text-foreground">Stockout Risk</div>
            <div className="mt-1 text-sm text-muted-foreground">Low-on-hand items and projected stockouts.</div>
          </div>
        </div>
      </SectionCard>
    </div>
