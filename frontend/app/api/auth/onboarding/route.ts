...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, profile: r.rows[0] });
  } catch (e: any) {
...
export async function POST(req: Request) {
  ...
  if (!first_name) return NextResponse.json({ ok: false, error: "first_name is required" }, { status: 400 });
  if (!last_name) return NextResponse.json({ ok: false, error: "last_name is required" }, { status: 400 });
  ...
  try {
    ...
    return NextResponse.json({ ok: true, redirect: "/onboarding/tenant" });
  } catch (e: any) {
    return NextResponse.json(
